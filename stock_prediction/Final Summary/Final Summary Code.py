...
COMPANY = 'AMZN'          # Company name to read
DATA_PATH = f"cache/{COMPANY.lower()}_all.csv"


TRAIN_START = '2024-08-01'     # Start date to read
TRAIN_END = '2025-09-01'       # End date to read
...
LOOKBACK = 60
HORIZON  = 1
CACHE_NPZ = f"cache/c2v1_{COMPANY.lower()}.npz"
...
ohlc_df = pd.read_csv("DATA_PATH")  # ajusta la ruta si usas otra
ohlc_df = prepare_ohlc(ohlc_df, date_col="date")

# Daily candles
plot_candles_mpf(ohlc_df, title=f"{COMPANY} – Daily Candles (mplfinance)")

# 5-day candles
ohlc_5d = resample_ohlcv(ohlc_df, freq="5D")
plot_candles_mpf(ohlc_5d, title=f"{COMPANY} – 5D Candles (mplfinance)")

# Plotly candles Plotly
plot_candles_plotly(ohlc_df, title=f"{COMPANY} – Daily (Plotly)")

# 5-day candles Plotly
plot_candles_plotly(resample_ohlcv(ohlc_df, "5D"), title=f"{COMPANY} – 5D (Plotly)")

# Rolling boxplots
plot_boxplot_rolling(ohlc_df, value_col="close", window=5, step=5,
                     title=f"{COMPANY} – Boxplot rolling (5 días)")
...
def run_c5_example(csv_path="DATA_PATH",
...
# --- Quick C5 demo ---
C5_example= run_c5_example(csv_path="DATA_PATH", lookback=60, horizon=5, n_splits=3, test_ratio=0.2)
...
def load_sentiment_data(csv_prices: str = "DATA_PATH",
...
def run_c7_experiment(csv_prices: str = "DATA_PATH",
...
if __name__ == "__main__":
    res_c7 = run_c7_experiment("DATA_PATH")
    print("Final metrics (C7):", res_c7["metrics"])

# --- Next-day prediction ---
    model = res_c7["model"]
    scaler = res_c7["scaler"]
    prob, trend = predict_next_price("DATA_PATH", model=model, scaler=scaler)
